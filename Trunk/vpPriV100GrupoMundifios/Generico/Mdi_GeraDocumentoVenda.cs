...
namespace Generico
{
    internal class Mdi_GeraDocumentoVenda
    {
        ...
        private static ErpBS BSO = new ErpBS();
        ...
        public static bool GerarDocumento_BaseVendas(VndBE100.VndBEDocumentoVenda DocumentoBase, string BaseDadosDestino, string TipoDoc_Destino, string Serie_Destino, string Entidade_Destino, string Armazem_Destino, double ValorASomarArtigo)
        {
            try
            {
                ...
                int preenche = 5;
                BSO.Vendas.Documentos.PreencheDadosRelacionados(DocumentoNovo_Venda, ref preenche);
                ...
                if (TipoDoc_Destino == "GR" && DocumentoBase.Tipodoc == "GR")
                {
                    ...
                    Lst_MoradaDescarga = BSO.Consulta(Str_MoradaDescarga);
                    ...
                    DocumentoBase_Encomenda = BSO.Vendas.Documentos.Edita("000", TipoDocFinal, SerieFinal, NumDocFinal);
                ...
                else
                {
                    if (DocumentoBase.CamposUtil["CDU_MoradaAlternativa"].Valor + "" != "")
                    {
                        ...
                        DocumentoNovo_Venda.CargaDescarga.PaisEntrega = BSO.Base.MoradasAlternativas.DaValorAtributo("C", DocumentoNovo_Venda.Entidade, DocumentoBase.CamposUtil["CDU_MoradaAlternativa"].Valor.ToString(), "Pais");
                        DocumentoNovo_Venda.CargaDescarga.MoradaEntrega = BSO.Base.MoradasAlternativas.Edita("C", DocumentoNovo_Venda.Entidade, DocumentoBase.CamposUtil["CDU_MoradaAlternativa"].Valor.ToString()).Morada;
                        DocumentoNovo_Venda.UtilizaMoradaAlternativaEntreg = true;
                    }
                }
                ...
                BSO.Vendas.Documentos.AdicionaLinhaEspecial(DocumentoNovo_Venda, BasBE100.BasBETiposGcp.vdTipoLinhaEspecial.vdLinha_Comentario, Descricao: "@1@ @2@/N.º@3@ de @4@");
                ...
                for (i = 1; i <= DocumentoBase.Linhas.NumItens; i++)
                {
                    if (DocumentoBase.Linhas.GetEdita(i).TipoLinha == "60")
                    {
                        ...
                        {
                            BSO.Vendas.Documentos.AdicionaLinhaEspecial(DocumentoNovo_Venda, BasBE100.BasBETiposGcp.vdTipoLinhaEspecial.vdLinha_Comentario, Descricao: DocumentoBase.Linhas.GetEdita(i).Descricao);
                    ...
                    else
                    {
                        ...
                        BSO.Vendas.Documentos.AdicionaLinha(DocumentoNovo_Venda, Artigo, ref Quantidade, ref Armazem, ref Localizacao);
                ...
                BSO.Vendas.Documentos.Actualiza(DocumentoNovo_Venda);
                ...
                if (DocumentoNovo_Venda.Tipodoc == "GR" & DateAndTime.DateDiff("d", DocumentoNovo_Venda.DataDoc, DocumentoNovo_Venda.DataVenc) < 9L)
                {
                    ...
                    VarAssunto = "Emitir Fatura: " + DocumentoNovo_Venda.Tipodoc + " " + Strings.Format(DocumentoNovo_Venda.NumDoc, "####") + "/" + DocumentoNovo_Venda.Serie + " (" + Strings.Replace(BSO.Base.Clientes.Edita(VarCliente).Nome, "'", "") + ")";
                    VarUtilizador = BSO.Contexto.UtilizadorActual;
                    VarMensagem = VarTextoInicialMsg + '\r' + '\r' + '\r' + "Foi emitido uma Guia com prazo de pagamento inferior ou igual a 8 dias, por favor emita a respetiva fatura:" + '\r' + '\r' + "" + "Empresa:                         " + BaseDadosDestino + '\r' + "" + "Utilizador:                      " + VarUtilizador + '\r' + '\r' + "" + "Cliente:                         " + VarCliente + " - " + Strings.Replace(BSO.Base.Clientes.Edita(VarCliente).Nome, "'", "") + '\r' + "" + "Documento:                       " + DocumentoNovo_Venda.Tipodoc + " " + Strings.Format(DocumentoNovo_Venda.NumDoc, "#,###") + "/" + DocumentoNovo_Venda.Serie + '\r' + '\r' + "" + "Data Vencimento:                 " + DocumentoNovo_Venda.DataVenc + '\r' + '\r' + "" + "Local Descarga:                  " + DocumentoNovo_Venda.LocalDescarga + '\r' + "" + "Morada Entrega:                  " + Strings.Replace(DocumentoNovo_Venda.MoradaEntrega, "'", "") + '\r' + '\r' + "" + "Cumprimentos";
                ...
                for (int k = 1; k <= DocumentoNovo_Venda.Linhas.NumItens; k++)
                {
                    ...
                    {
                        BSO.DSO.ExecuteSQL("update LinhasDoc set CDU_IDLinhaOriginalGrupo = '" + Strings.Mid(IDLinhaDocOriginal[k], 1, Strings.InStr(1, IDLinhaDocOriginal[k], ";") - 1) + "' where  id = '" + Strings.Mid(IDLinhaDocOriginal[k], Strings.InStr(1, IDLinhaDocOriginal[k], ";") + 1, Strings.Len(IDLinhaDocOriginal[k])) + "' ");
                    }
                }

                try
                {
                    BSO.DSO.ExecuteSQL(" UPDATE CabecDoc " + " SET CDU_DocumentoVendaDestino = '" + DocumentoNovo_Venda.Tipodoc + " " + DocumentoNovo_Venda.Serie + "/" + DocumentoNovo_Venda.NumDoc + "' " + " where filial = '" + DocumentoBase.Filial + "' and  TipoDoc = '" + DocumentoBase.Tipodoc + "' and Serie = '" + DocumentoBase.Serie + "' and NumDoc = " + DocumentoBase.NumDoc + " ");
        ...
        private static bool Rastreabilidade(string Filial_FaturaCliente, string Serie_FaturaCliente, string TipoDoc_FaturaCliente, long NumDoc_FaturaCliente, string BaseDadosDestino, VndBEDocumentoVenda FaturaClienteFinal_AcabadoGerar)
        {
            ...
            if (Lst_StrRastreabilidade.Vazia() == false)
            {
                ...
                while (!Lst_StrRastreabilidade.NoFim())
                {
                    ...
                    BSO.DSO.ExecuteSQL(" INSERT INTO LinhasDocTrans " + " (IdLinhasDoc,IdLinhasDocOrigem,QuantTrans) " + " VALUES( '" + Lst_StrRastreabilidade.Valor("IdLinhasDocFaturaGrupo") + "','" + Lst_StrRastreabilidade.Valor("IdLinhasDocEncomendaGrupo") + "'," + Strings.Replace(Lst_StrRastreabilidade.Valor("QuantidadeEncomendaGrupo"), ",", ".") + " ) ");
                    Lst_StrRastreabilidade.Seguinte();
                }

                if (BSO.Base.Series.Edita("V", DocumentoNovo_Venda.Tipodoc, DocumentoNovo_Venda.Serie).TipoComunicacao == 2)
                    BSO.Internos.Documentos.ATComunicaDocumentoId(DocumentoNovo_Venda.ID, "V", ref ErroComunica);
        ...
        {
            FaturaClienteFinal_AcabadoGerar = BSO.Vendas.Documentos.Edita(FaturaClienteFinal_AcabadoGerar.Filial, FaturaClienteFinal_AcabadoGerar.Tipodoc, FaturaClienteFinal_AcabadoGerar.Serie, FaturaClienteFinal_AcabadoGerar.NumDoc);
            ...
            BSO.Vendas.Documentos.Actualiza(FaturaClienteFinal_AcabadoGerar);
        }
    }
}